...
const {getMapKeyForNode, normalizeWhitespace} = require('../helpers');
...
module.exports.action = function (path, scope) {
  ...
  if (/(column|columns|container|image)/.test(parent.$$kind)) {
    if (parent.selectedAction) {
      throw path.buildCodeFrameError(`Only a single action can exist on the ${parent.$$kind} element`);
    }
    parent.selectedAction = action;
  } else if (!insertElementViaCommonParentProps(action, parent)) {
    path.buildCodeFrameError(`${parent.$$kind} must not contain an action element`);
  }
  return action;
};
...
module.exports.actions = function (path, scope) {
  ...
  if (path.node.openingElement.attributes.length) {
    throw path.buildCodeFrameError('The actions element cannot contain attributes');
  }
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.card) {
    parent.actions = actions;
  } else if (!insertElementViaCommonParentProps(actions, parent)) {
    throw path.buildCodeFrameError(`The actions element cannot be used with the ${parent.type} element`);
  }
  return actions;
};
...
module.exports.body = function (path, scope) {
  ...
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.card) {
    ...
    throw path.buildCodeFrameError(`The body element cannot be used with the ${parent.type} element`);
  }
...
module.exports.card = function (path, scope) {
  ...
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.action.showCard) {
    ...
    throw path.buildCodeFrameError('The card element is not allowed here');
  }
...
module.exports.choice = function (path, scope) {
  ...
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.input.choiceSet) {
    ...
    throw path.buildCodeFrameError('The choice element can only exist as a child of the <input type="choiceSet"/> tag');
  return choice;
};
...
module.exports.column = function (path, scope) {
  ...
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.columns) {
    ...
    throw path.buildCodeFrameError('The column element can only exist as children in the columns tag');
  }
...
module.exports.columns = function (path, scope) {
  ...
  if (parent && parent.attrs && parent.attrs.type === definitionsMap.container) {
    ...
    throw path.buildCodeFrameError('The columns element can only exist as children in the body or container tag');
  }
  return columns;
};
...
module.exports.fact = function (path, scope) {
  ...
  if (!insertElementViaCommonParentProps(fact, parent)) {
    throw path.buildCodeFrameError('The fact element can only exist as a child in a facts element');
  }
  return fact;
};
...
module.exports.facts = function (path, scope) {
  ...
  if (/(column|container)/.test(parent.$$kind)) {
    ...
    throw path.buildCodeFrameError(`The facts element can only exist as children in the body or container tag`);
  }
  return facts.facts;
};
...
module.exports.image = function (path, scope) {
  ...
  if (parent.attrs && parent.attrs.type === definitionsMap.images) {
    ...
    throw path.buildCodeFrameError('The image element can only exist as children in a component or the body or container tag');
  }
...
module.exports.input = function (path, scope) {
  ...
  if (children && children.length) {
    switch (input.attrs.type) {
      ...
      default:
        throw path.buildCodeFrameError(`The input[type="${input.attrs.type}"] cannot contain children`);
    }
  }

  if (!insertElementViaCommonParentProps(input, parent)) {
    throw path.buildCodeFrameError('The input element can only exist as children in the body or container tag');
  }
...
module.exports.text = function (path, scope) {
  ...
  if (parent && /(column|container)/.test(parent.$$kind)) {
    ...
    throw path.buildCodeFrameError('The text element does not belong here');
  }
  return text;
};
...
function copyAttributes(path, destination) {
  ...
  if (typeAttr && !t.isStringLiteral(typeAttr.value)) {
    throw path.buildCodeFrameError('Type attributes must be string literals');
  }
  ...
  if (!definition.properties && path.node.openingElement.attributes.length) {
    throw path.buildCodeFrameError(`${destination.$$kind} does not allow attributes`);
  }
  ...
  (path.node.openingElement.attributes || []).forEach(attr => {
    if (!t.isJSXSpreadAttribute(attr)) {
      ...
      if (!definition.properties[name] || (destination[name] && destination.attrs[name])) {
        throw path.buildCodeFrameError(`The ${name} attribute is not allowed here`);
      }
